...
final class TipCalculatorTests: XCTestCase {
    ...
    func testResultWithTip10PercentFor2Person() {
        //given
        let bill: Double = 100.0
    ...
    func testSoundPlayedAndCalculatorResetOnLogoViewTap() {
        //given
        let input = buildInput(bill: 100, tip: .fifteenPercent, split: 4)
        let output = sut.transform(input: input)
        let expectation1 = XCTestExpectation(description: "reset calculator called")
        let expectation2 = audioPlayerService.expection
...
class MockAudioPlayerService: AudioPlayerService {
    var expection = XCTestExpectation(description: "audioplayer is called")
    func playSound() {
        expection.fulfill()
    }
}
